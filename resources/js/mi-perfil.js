...
function comprobarBoletin() {
  ...
  $.post(
    "controlador/ajax/aula-virtual-Est2.php",
...
function data_load() {
  $.ajax({
    ...
    document.getElementById("nombre_perfil").textContent =
    ...
    document.getElementById("ruta_imagen").src =
      objeto.ruta_imagen === ""
        ? "resources/img/nothingPhoto.png"
        : objeto.ruta_imagen;
...
const expresiones = {
  //objeto con varias expresiones regulares
  cedula: /^[0-9]{7,8}$/,
...
const validarFecha_nacimiento = (input, campo) => {
  ...
  if (
    fechaNacimientoTS < mayoriaEdad.getTime() &&
    fechaNacimientoTS > maximaEdad.getTime()
  ) {
    document
    ...
    document
      .querySelector(`#grupo__${campo} i`)
      .classList.add("bi", "bi-check-circle-fill", "text-check", "input-icon2");
...
const ValidarSelect = (select, campo) => {
  if (select.value == "") {
    ...
    document.querySelector(`#grupo__${campo} p`).classList.add("d-block");
    document
      .querySelector(`#grupo__${campo} select`)
...
const ValidarCampo = (expresion, input, campo) => {
  if (expresion.test(input.value)) {
    document.querySelector(`#grupo__${campo} p`).classList.remove("d-block");
    document
      .querySelector(`#grupo__${campo} input`)
      .classList.remove("is-invalid");
    ...
    if (campos.cedula == true) {
      ...
      $.ajax({
        ...
        if (data == "1") {
          ...
          mensaje.textContent =
            "Esta cedula ya existe en la base de datos, ingrese otra por favor";
        }
      });
    }
    if (campos.correo == true) {
      ...
      $.ajax({
        ...
        if (data == "1") {
          ...
          mensaje.textContent =
            "Esta correo ya existe en la base de datos, ingrese otro por favor";
        }
      });
    }
  } else {
...
formulario.addEventListener("submit", (e) => {
  ...
  if (
    ...
    fire_alerta_problem(
      "Lo siento",
      "Registra el formulario correctamente",
      "error"
    );
  }
});
